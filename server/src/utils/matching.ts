...
const assignPreferenceScores = async function(userId, callback) {
    ...
    try {
        ...
        groups.forEach(function(group) {
            ...
            const tasks = [
                    ...
                    calcPcntIntersect(
                        user.schedule,
                        group.meeting_times,
                        (err, percent_intersect, meetings) => {
                            if (err) {
                                done(err);
                            } else {
        ...
        let sorted_potential_matches = potential_matches.sort(
            (pm1, pm2) => pm2.pref - pm1.pref
        );
...
const findGroupForUser = async function(
    userId,
    sortedPotentialMatches,
    callback
) {
...
const joinGroup = async function(userId, groupId, callback) {
    try {
        ...
        calcPcntIntersect(
            user.schedule,
            group.meeting_times,
            (err, pctIntersect, new_meeting_times) => {
                if (err) return callback(err);
                else {
        ...
        for (let i = 0; i < group.members.length; i++) {
            ...
            if (curr_member) {
                groupCopy.names.push(
                    `${curr_member.firstName} ${curr_member.lastName}`
                );
